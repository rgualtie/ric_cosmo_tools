...
parser.add_argument('--disk',type=int, default=4,
                   help = 'Disk space requested in GB, default=4 GB')
parser.add_argument('--memory',type=int, default=4,
                   help = 'RAM requested in GB, default=4 GB')
parser.add_argument('--bkmask', action = 'store_true',
                   help = "Calculate using bk14 mask")
parser.add_argument('--binw', type=str, default='25',
                   help = 'Bin width to calculate the spectra, default = 25')
...
dag_script = os.path.join(condor_dir, map_name+'_'+pargs.band+'GHz.dag')
...
if pargs.bkmask:
    mask = '/home/umilta/bk14_mask_gal_n0512.fits'
    output_folder = os.path.join(grid_path, 'output_spectra_bkmask/'+pargs.band+'GHz')
else:
    mask = '/sptgrid/user/rgualtie/ApodMaskPS2048.fits'
    output_folder = os.path.join(grid_path, 'output_spectra/'+pargs.band+'GHz')
jobname = 'spectra_'+map_name+'_'+pargs.band+'GHz_nstubs'+pargs.nobs
args = "--inmap coadd_"+base_input_file+" --mask "+os.path.basename(mask)+" --output "+jobname+'.npz --binw '+pargs.binw
...
condor_submit(script, create_only=create_only, args = [args],
          ...
          retry = True,
          jobname = jobname,
          input_files = [job_root+'/'+pargs.band+'GHz'+'/coadd_'+base_input_file],
          ignore_input = True,
