...
parser.add_argument('--nside', type=str, default='4096')
parser.add_argument('--band', type=str, default='90')
parser.add_argument('--nobs', type=str, default='512',
                   help = 'Number of ObsIDs to be used, active if --randomobs is used, default=512')
...
os.system('python src/submit_mock_obs_map_dag_2.0.py --group LeakageTemplate --dag --inmap '+pargs.inmap+' --healpix --nside '+pargs.nside+' --band '+pargs.band+' --randomobs --nobs '+pargs.nobs)

# Coadd the fields
coadd_mapname = os.path.splitext(os.path.basename(pargs.inmap))[0]
os.system('python src/submit_coadder_stdproc_2.0.py --nobs '+pargs.nobs+' --inmap '+coadd_mapname+' --dag --group LeakageTemplate --band '+pargs.band)

# Calculate the power spectra
os.system('python src/submit_power_spectra_2.0.py --inmap /sptgrid/user/'+os.environ.get('USER')+'/LeakageTemplate/'+coadd_mapname+'/'+coadd_mapname+'_'+pargs.band+'GHz_nstubs'+pargs.nobs+'.g3 --dag --group LeakageTemplate --band '+pargs.band+' --nobs '+pargs.nobs)
...
with open(dag_script,'a') as f:
    ...
    if pargs.cleanup:
        ...
        f.write('SCRIPT POST spectra_'+os.path.splitext(os.path.basename(pargs.inmap))[0]+'_'+pargs.band+'GHz_nstubs'+pargs.nobs+' /home/'+os.environ.get('USER')+'/src/cleanup_PTonly.sh '+os.path.splitext(os.path.basename(pargs.inmap))[0]+' '+pargs.band)
