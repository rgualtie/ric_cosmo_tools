...
os.system('python src/submit_mock_obs_map_dag_2.0.py --group mix_matrix --dag --inmap '+pargs.inmap+' --healpix --nside '+pargs.nside+' --band '+pargs.band+' --randomobs --nobs '+pargs.nobs)

# Coadd the fields
coadd_mapname = os.path.splitext(os.path.basename(pargs.inmap))[0]
os.system('python src/submit_coadder_stdproc_2.0.py --nobs '+pargs.nobs+' --inmap '+coadd_mapname+' --dag --group mix_matrix --band '+pargs.band)

# Calculate the power spectra
os.system('python src/submit_power_spectra_2.0.py --inmap /sptgrid/user/'+os.environ.get('USER')+'/mix_matrix/'+coadd_mapname+'/'+coadd_mapname+'_'+pargs.band+'GHz_nstubs'+pargs.nobs+'.g3 --dag --group mix_matrix --band '+pargs.band+' --nobs '+pargs.nobs)
...
with open(dag_script,'a') as f:
    ...
    if pargs.cleanup:
        ...
        f.write('SCRIPT POST spectra_'+os.path.splitext(os.path.basename(pargs.inmap))[0]+'_'+pargs.band+'GHz_nstubs'+pargs.nobs+' /home/'+os.environ.get('USER')+'/src/cleanup_mixmat_2.0.sh '+os.path.splitext(os.path.basename(pargs.inmap))[0]+' '+pargs.band)
