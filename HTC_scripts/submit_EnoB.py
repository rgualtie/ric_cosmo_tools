...
os.system('python src/submit_mock_obs_map_dag.py --group EnoB --dag --inmap '+pargs.inmap+' --healpix --nside '+pargs.nside+' --band '+pargs.band+' --randomobs --nobs '+pargs.nobs+' --remove_duplicates')
os.system('python src/submit_mock_obs_map_dag.py --group EnoB --dag --inmap '+pargs.inmap+' --healpix --nside '+pargs.nside+' --band '+pargs.band2+' --randomobs --nobs '+pargs.nobs+' --remove_duplicates')

# Coadd the fields
coadd_mapname = os.path.splitext(os.path.basename(pargs.inmap))[0]

os.system('python src/submit_coadder_stdproc.py --nobs '+pargs.nobs+' --inmap '+coadd_mapname+' --dag --group EnoB --band '+pargs.band)
os.system('python src/submit_coadder_stdproc.py --nobs '+pargs.nobs+' --inmap '+coadd_mapname+' --dag --group EnoB --band '+pargs.band2)

os.system('python src/submit_cross_spectra.py --inmap /sptgrid/user/'+os.environ.get('USER')+'/EnoB/'+coadd_mapname+'/'+coadd_mapname+'_'+pargs.band+'GHz_nstubs'+pargs.nobs+'.g3 --inmap2 /sptgrid/user/'+os.environ.get('USER')+'/EnoB/'+coadd_mapname+'/'+coadd_mapname+'_'+pargs.band2+'GHz_nstubs'+pargs.nobs+'.g3 --dag --group EnoB --band '+pargs.band+' --band2 '+pargs.band2+' --nobs '+pargs.nobs)
