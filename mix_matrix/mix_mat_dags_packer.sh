...
for f in 90;do for s in EE;do for b in 133 183;do for i in {00..99};do python src/mix_matrix_parallel_2.0.py --nside 512 --infile /sptgrid/user/rgualtie/mix_matrix/input_spectra/spectra_$s\_0$b.npy --band $f --simidx $i; echo "$i: Waiting 1sec.."; sleep 1; done; done; done; done
